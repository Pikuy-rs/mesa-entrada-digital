...
import CatedraRadarChart from './CatedraRadarChart';
...
const AXES = [
  {
    ...
    description: 'Evalúa la capacidad de la cátedra para transmitir conocimientos de forma clara y aplicada.',
    questions: [
      { id: 'ict_1', text: '¿Se demuestra dominio profundo de la materia?' },
      { id: 'ict_2', text: '¿Se utilizan ejemplos prácticos aplicables a la ingeniería?' },
      { id: 'ict_3', text: '¿Se fomenta el pensamiento crítico y la participación?' }
    ]
  },
  {
    id: 'ndc',
  ...
  },
  {
    ...
    questions: [
      { id: 'cat_1', text: '¿Se mantiene un trato cordial, respetuoso y empático?' },
      { id: 'cat_2', text: '¿Se cumplen estrictamente con los horarios de inicio y fin de clase?' },
      { id: 'cat_3', text: '¿Existe predisposición para resolver dudas (presencial o virtual)?' }
    ]
  },
  {
    id: 'tce',
...
export default function AcademicExcellence() {
  ...
  const [activeCatedra, setActiveCatedra] = useState(null);
  ...
  useEffect(() => {
    const fetchCatedras = async () => {
      const dbCatedras = await evaluacionSkill.getCatedras();
      const combined = [...VIRTUAL_CURRICULUM];
      
      dbCatedras.forEach(dbC => {
        const index = combined.findIndex(v => v.catedra === dbC.catedra && v.carrera === dbC.carrera);
        if (index !== -1) {
          combined[index] = dbC;
        } else {
          combined.push(dbC);
      ...
      setCatedrasList(combined);
    };
    fetchCatedras();
  }, []);

  const uniqueCatedras = catedrasList
  ...
  useEffect(() => {
    if (selectedCatedra) {
      const catedra = catedrasList.find(t => t.catedra === selectedCatedra && t.carrera === selectedCarrera);
      if (catedra) setActiveCatedra(catedra);
    } else {
      setActiveCatedra(null);
    }
  }, [selectedCatedra, catedrasList, selectedCarrera]);
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await evaluacionSkill.submit(activeCatedra.id, evaluationData, activeCatedra);
  ...
  return (
    <section id="excelencia" className={styles.section}>
      <div className="container">
        <h2 style={{ fontSize: '3rem', fontWeight: '800', color: 'var(--color-primary)', textAlign: 'center', marginBottom: '16px' }} className="text-gradient-primary">
          Excelencia Académica
        </h2>
        <p className={styles.subtitle} style={{ marginBottom: '48px' }}>Monitor Estratégico de Calidad Educativa - Gestión Directa AT</p>

        <div style={{ marginBottom: '64px' }}><DataFlowTimeline /></div>

        <div style={{ maxWidth: '1100px', margin: '0 auto 60px auto', padding: '60px', background: '#ffffff', borderRadius: '3rem', border: '5px solid #3f75ab', boxShadow: '0 40px 80px rgba(0,0,0,0.12)', position: 'relative' }} className="animate-fade-in">
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(260px, 1fr) 2fr', gap: '60px', alignItems: 'center' }}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ width: '260px', height: '260px', borderRadius: '50%', overflow: 'hidden', border: '8px solid #3f75ab', boxShadow: '0 20px 40px rgba(63, 117, 171, 0.3)', margin: '0 auto 24px auto' }}>
                <Image src="/gala.png" alt="Gala Bórquez" width={260} height={260} style={{ objectFit: 'cover' }} />
              </div>
            </div>
            <div>
              <div style={{ paddingRight: '20px' }}>
                <p style={{ fontSize: '1.45rem', color: '#000000', lineHeight: '1.6', fontWeight: '700', fontStyle: 'italic', margin: '0 0 32px 0', letterSpacing: '-0.01em' }}>
                  "En Alternativa Tecnológica entendemos que la educación de excelencia es un diálogo constante. El Sistema de Retroalimentación de Excelencia no nace para juzgar, sino para construir puentes. Al Estudiante: Le damos la voz para proteger el valor de su título. A la Cátedra: Le brindamos una métrica real y profesional de su impacto en el aula. Creemos firmemente que una observación, una sugerencia o incluso una queja, son indicadores valiosos para fortalecer la enseñanza."
                </p>
                <div style={{ borderTop: '2px solid #f3f4f6', paddingTop: '20px', textAlign: 'right' }}>
                  <h3 style={{ fontSize: '1.8rem', color: '#3f75ab', margin: '0 0 4px 0', fontWeight: '950' }}>Gala Bórquez</h3>
                  <span style={{ fontSize: '1.1rem', color: '#4b5563', fontWeight: '800', textTransform: 'uppercase' }}>Presidenta del Centro de Estudiantes</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {!showForm && (
          <div style={{ textAlign: 'center', marginBottom: '80px' }}>
            <button className="glass-button" style={{ fontSize: '1.8rem', padding: '24px 80px', borderRadius: '1.5rem', background: '#3f75ab', color: '#fff', width: '100%', maxWidth: '500px' }} onClick={() => setShowForm(true)}>
        ...
        {showForm && (
          <div className="animate-fade-in-up" style={{ maxWidth: '1000px', margin: '0 auto', padding: '60px', background: '#ffffff', border: '2px solid #3f75ab', borderRadius: '2.5rem', boxShadow: '0 40px 80px rgba(0,0,0,0.18)' }}>
            {status === 'success' ? (
              <div style={{ textAlign: 'center', padding: '60px 0' }}>
                ...
                <button className="glass-button" style={{ marginTop: '32px', background: '#3f75ab', color: '#fff', padding: '12px 32px', borderRadius: '1rem' }} onClick={() => setStatus('idle')}>Evaluar otra cátedra</button>
              </div>
            ) : (
              <form onSubmit={handleSubmit}>
                <div style={{ background: '#f9fafb', padding: '40px', borderRadius: '2rem', border: '1px solid #e5e7eb', marginBottom: '48px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                    <div>
                      <label style={{ display: 'block', fontWeight: '800', marginBottom: '12px' }}>Carrera</label>
                      <select className="glass-input" value={selectedCarrera} disabled style={{ width: '100%', background: '#ffffff', border: '2px solid #3f75ab', fontWeight: '700' }}>
                        <option value="Ingeniería en Sistemas de Información">Ingeniería en Sistemas de Información</option>
                      </select>
                    </div>
                    <div>
                      <label style={{ display: 'block', fontWeight: '800', marginBottom: '12px' }}>Cátedra / Materia</label>
                      <select className="glass-input" value={selectedCatedra} onChange={(e) => setSelectedCatedra(e.target.value)} style={{ width: '100%', background: '#ffffff', border: '2px solid #3f75ab', fontWeight: '700' }}>
                        <option value="">Selecciona la materia...</option>
                        {uniqueCatedras.map(cat => <option key={cat} value={cat}>{cat}</option>)}
                      </select>
                    </div>
                  </div>
                </div>
                ...
                {!activeCatedra ? (
                  ...
                  <div className="animate-fade-in">
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '40px' }}>
                      {AXES.map((axis) => (
                        <div key={axis.id} style={{ background: '#ffffff', padding: '40px', borderRadius: '2rem', border: '1px solid #e5e7eb' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
                            <h4 style={{ margin: 0, fontSize: '1.4rem', color: '#3f75ab', fontWeight: '900' }}>{axis.title}</h4>
                            <div className="tooltip-container" title={axis.description} style={{ color: '#ef5f27' }}><Info size={22} /></div>
                          </div>
                          <p style={{ fontSize: '1rem', color: '#4b5563', marginBottom: '24px', borderBottom: '2px solid #f3f4f6', paddingBottom: '16px', fontWeight: '600' }}>{axis.description}</p>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                            {axis.questions.map((q) => (
                              <div key={q.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '24px', flexWrap: 'wrap' }}>
                                <span style={{ fontSize: '1.1rem', flex: '1 1 300px', fontWeight: '700' }}>{q.text}</span>
                    ...
                    <div style={{ marginTop: '48px', background: '#ffffff', padding: '40px', borderRadius: '2.5rem', border: '3px solid #3f75ab' }}>
                      <label style={{ display: 'block', fontWeight: 950, marginBottom: '8px', fontSize: '1.6rem' }}>Acción para la Excelencia</label>
                      <textarea className="glass-input" rows="6" placeholder="Deja aquí tus sugerencias..." value={formData.accionExcelencia} onChange={(e) => setFormData(prev => ({ ...prev, accionExcelencia: e.target.value }))} style={{ width: '100%', background: '#ffffff', padding: '20px', fontSize: '1.2rem', borderRadius: '1rem' }} />
                    </div>

                    {status === 'error' && <div style={{ color: '#ac55a4', background: 'rgba(172, 85, 164, 0.1)', padding: '16px', borderRadius: '8px', marginTop: '24px', fontWeight: '700' }}>{errorMsg}</div>}
